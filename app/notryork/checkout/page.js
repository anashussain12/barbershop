...
export default function CheckoutPage() {
  ...
  const getSelectedServices = () => {
    return Object.values(selectedOptions).reduce((acc, section) => {
      return [...acc, ...Object.values(section).filter(service => typeof service === 'object')];
    }, []);
  };
                              ...
                              >
                                <div className="flex items-center gap-2">
                                  <input
                                    type="checkbox"
                                    checked={
                                      !!selectedOptions[section.title]?.[option.name]
                                    }
                                    onChange={() =>
                                      handleCheckboxChange(section.title, option)
                                    }
