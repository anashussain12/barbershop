...
export default function CheckoutPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    // Check if there is an existing booking with the same details and status "pending"
    const bookingsRef = collection(db, "bookings");
    const q = query(
      bookingsRef,
      where("firstName", "==", formData.firstName),
      ...
      where("date", "==", formData.date),
      where("phone", "==", formData.phone),
      where("status", "==", "pending") // Ensure the status is 'pending'
    );

    const querySnapshot = await getDocs(q);
    if (!querySnapshot.empty) {
      // If a matching booking is found with status 'pending', show a message and prevent the booking
      alert(
    ...
    try {
      // Add the new booking with status 'pending'
      const docRef = await addDoc(collection(db, "bookings"), {
        ...form,
        ...
        <div className="max-w-4xl mx-auto">
          ...
          <div className="text-center mb-12">
            ...
            <p className="text-gray-300 max-w-2xl mx-auto">
              ...
              your booking at our Ajman location.
            </p>
          </div>
